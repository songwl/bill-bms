...
<div id="offerSetUp" style="display: none;">
    ...
    <div class="modal-body">
        ...
        <div class="form-group">
            <input type="text" class="form-control setkeyword" id="keywordNum" placeholder="请输入关键词数量" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')">
        </div>
        <button id="confim" class="btn btn-success form-control"><span class="glyphicon glyphicon-send">&nbsp;</span><span>确认</span></button>
    </div>
</div>
<!--报价设置end-->
</@base.html>